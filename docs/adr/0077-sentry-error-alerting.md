...
The one piece of context worth having — which tenant hit the error — is attached as `Sentry.setTag('userId', userId)`, not `Sentry.setUser(...)`. A tag is an opaque string with no special handling; `setUser` is the API conceptually tied to the "user info" PII category and would be the wrong thing to reach for if that category's defaults ever change.
...
- `environment:` is **not** a new env var. API reads Railway's own auto-injected `RAILWAY_ENVIRONMENT_NAME`, which — since ADR-0075 renamed Railway environments to exactly `prod`/`preprod` — already resolves to the right string with nothing to configure. Web reuses the existing `VITE_ENVIRONMENT` (unset → `prod`, `'preprod'` on the preprod Vercel project), the same var `apps/web/src/lib/environment.ts` already reads for the UI badge.
...
- Two new `.env.example` entries: `SENTRY_DSN`, `VITE_SENTRY_DSN` — off locally, set in preprod and prod.
- Manual, human-only post-merge steps (not blocking the code itself, same shape as #785's prod-var follow-up): set `SENTRY_DSN`/`VITE_SENTRY_DSN` in Railway/Vercel for preprod and prod; confirm `VITE_SENTRY_DSN` is **not** marked Sensitive in Vercel (the #733 trap — a Sensitive `VITE_` var bakes as `""` and the feature silently never turns on); confirm the Sentry project's alert rule is scoped to `environment: prod`.
