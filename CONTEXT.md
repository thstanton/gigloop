...
**Goal states:** `PENDING | COMPLETE | FAILED | SKIPPED`. For a multi-step goal `state` is a roll-up of its [[Step]]s; **`SKIPPED` is the one state set directly by the musician** — it is the opt-out (toggling the goal off in the "Remind me about" control, reversible). There is no `BLOCKED`: inter-goal ordering is *soft* (status order), never a hard lock.
...
Also carries `completedBy: USER | CUSTOMER | BAND_MEMBER` (routes who acts). **Step states:** `PENDING | COMPLETE | FAILED` — a step is never `SKIPPED` (the musician toggles goals, not steps) and never `BLOCKED` (the **active step is derived** — the first non-terminal step by `order` — so step inactivity is computed, not stored). `dependsOn` does not exist on steps: intra-goal sequencing is `order`. See [[Goal]] and ADR-0057.
