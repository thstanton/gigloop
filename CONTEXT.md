...
- **logistics** (optional JSON — see ADR-0034): all "on the day" operational fields in a single column. Keys are either system-defined (`arrivalTime`, `soundCheckTime`, `finishTime`, `dressCode`, `performanceSpace`, `foodProvided`, `greenRoom`, `equipmentRequired`) or user-defined custom fields. Each entry has the shape `{ value: string; icon?: string; shareWithBand: boolean; shareWithClient: boolean }`. Custom entries additionally carry `label: string`; their key is machine-generated (`customField1`, etc.). Both sharing flags default to `false`. System field display labels are derived from the key via a constants map; custom field labels are stored in the entry.
