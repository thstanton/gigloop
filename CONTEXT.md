...
**Design principle — itinerary vs packages:** There is **one** [[Set]] entity, viewed through two lenses that must not be conflated. The *operational* lens is the [[Booking]] itinerary — a time marker the musician cares about on the day. The *presentational* lens is the (booking-owned) [[Package]] grouping — an optional, client-facing way of organising sets into named segments (e.g. "Ceremony", "Evening Reception"), presented on the [[Portal]]. The package grouping is **not** a billing unit: the booking fee is authoritative and invoice line items are free-form. The [[ItineraryCard]] view expresses the operational concern (time + label, no package context); the Packages view expresses the grouping (sets under their package, with icon). Sets may be ungrouped. Editing a set in one view edits the single underlying record — there is nothing to sync. Mixing the two lenses in one view confuses the hierarchy.
...
### Package
A **booking-owned** grouping of [[Set]]s — the client-facing "set container" and the commercial lens on a booking's performance structure (see design principle *itinerary vs packages*). Created by applying a [[Package Template]] (which copies the template's slots into this Package as editable Sets) or **ad-hoc** (created directly, with sets added by hand). A Package is a **snapshot**: it carries its own `label`, `icon`, and `order`, copied at creation and **decoupled** from any template thereafter — editing a [[Package Template]] never alters an existing booking's Packages. **Provenance is severed** — a Package keeps no reference to the template it was built from. See ADR-0046.
