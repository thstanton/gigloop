...
import { apiGet, openPreviewPdf } from '@/lib/api';
...
export interface SeriesInvoiceSectionProps {
  ...
  isLoading: boolean;
  // The stored PDF's download route once the invoice is ISSUED+; null for a DRAFT (no document yet).
  pdfUrl: string | null;
...
export default function InvoiceSection({ bookingId }: Readonly<InvoiceSectionProps>) {
  ...
  function handlePreview(invoice: Invoice) {
    openPreviewPdf(`/bookings/${bookingId}/invoices/${invoice.id}/preview.pdf`, () =>
